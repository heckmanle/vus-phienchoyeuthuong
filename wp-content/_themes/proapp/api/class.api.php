...
class API {
    ...
    {
        //$this->url = 'http://116.118.112.247:8080/graphql';
        $this->url = API_URL;
        //$this->get_token();
    }

    function query($method, $func , $schemas = [], $token = false){

        $type = $method == 'POST' ? 'mutation' : 'query';

        $fields = isset($schemas['fields']) ? $this->parseFields($schemas['fields']) : '';
        $params = isset($schemas['params']) ? $this->parseParams($schemas['params']) : '';
        $query = $this->url . "?query=" . $type . "{" . $func . $params . $fields . "}";
        $args = [
            'headers' => array(
        ...
        $response = wp_remote_post($query, $args);
