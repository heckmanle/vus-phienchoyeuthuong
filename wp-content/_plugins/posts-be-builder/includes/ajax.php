...
class AJAX {
    ...
    {
        add_action('wp_ajax_postsbb_handle_ajax', [$this, 'handle_ajax_request']);
    }
    ...
    function process_action($callback, $data){
        try{
            ...
            if( $result === false ){
                ...
                $data = apply_filters('postsbb/ajax/insert_data_errors', []);
                $this->add_response_data($message, false, $data, $code)
                    ->send_error($code, $message);
            }else{
                $this->add_response_data(__('Success', REAL_ESTATE_PRODUCTS_LANG_DOMAIN), true, $result);
            }
        }catch (\Exception $e){
    ...
    function handle_ajax_request(){
        ...
        if( is_user_logged_in() ){
	        do_action('postsbb/ajax/register_actions', $this);
            $ajax_actions = isset($this->ajax_actions[$action]) ? $this->ajax_actions[$action] : [];
        }
        if( empty($ajax_actions) ){
            $this->add_response_data( false, __( 'Action not found.', REAL_ESTATE_PRODUCTS_LANG_DOMAIN ), [], 400 )
                ->send_error(400);
        }
    ...
    public function verify_request($id){
        if( !$this->verify_request_nonce($id) ){
            $message = __('Time has expired' ,REAL_ESTATE_PRODUCTS_LANG_DOMAIN );
    ...
    public function handle_ajax_action($tag, $callback){
        if ( ! did_action( 'postsbb/ajax/register_actions' ) ) {
            _doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'postsbb/ajax/register_actions' ) ), '1.0.0' );
        }
