...
if ( ! function_exists('pac_drh_add_class_to_shop')) {
    function pac_drh_add_class_to_shop($output, $render_slug, $module) {
        if (et_fb_is_enabled()) {
            return $output;
        }
        ...
        if ('off' !== $pac_drh_column_numbers_desktop) {
            if ($pac_drh_column_numbers_desktop === 'pac_drh_desktop_col_two' && (($posts_number % 2) === 1)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_desktop_2_col_last_r_1_element_fix', $output);
            }
            if ($pac_drh_column_numbers_desktop === 'pac_drh_desktop_col_three' && (($posts_number % 3) === 1)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_desktop_3_col_last_r_1_element_fix', $output);
            }
            if ($pac_drh_column_numbers_desktop === 'pac_drh_desktop_col_three' && (($posts_number % 3) === 2)) {
        ...
        if ('off' !== $pac_drh_column_numbers_tablet) {
            if ($pac_drh_column_numbers_tablet === 'pac_drh_tab_col_three' && (($posts_number % 3) === 1)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_tab_3_col_last_r_1_element_fix', $output);
            }
            if ($pac_drh_column_numbers_tablet === 'pac_drh_tab_col_three' && (($posts_number % 3) === 2)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_tab_3_col_last_r_2_element_fix', $output);
            }
            if ($pac_drh_column_numbers_tablet === 'pac_drh_tab_col_four' && (($posts_number % 4) === 1)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_tab_4_col_last_r_1_element_fix', $output);
            }
            if ($pac_drh_column_numbers_tablet === 'pac_drh_tab_col_four' && (($posts_number % 4) === 2)) {
                $output = preg_replace('/\bet_pb_module\b/', 'et_pb_module pac_drh_tab_4_col_last_r_2_element_fix', $output);
            }
            if ($pac_drh_column_numbers_tablet === 'pac_drh_tab_col_four' && (($posts_number % 4) === 3)) {
...
add_filter('et_module_shortcode_output', 'pac_drh_add_class_to_shop', 10, 3);

if ( ! function_exists('pac_drh_add_shop_col_numbering_options')) {
    function pac_drh_add_shop_col_numbering_options($fields_unprocessed) {
        $fields = [];
        $fields['pac_drh_column_numbers_desktop'] = [
            'label' => esc_html__('Number of Shop Columns on Desktop', 'Divi'),
            'type' => 'select',
            'options' => [
            ...
            'default' => 'off',
            'description' => esc_html__('Select the number of product columns to show on Desktop.', 'Divi'),
            'tab_slug' => 'general',
            'toggle_slug' => 'main_content',
        ];
        $fields['pac_drh_column_numbers_tablet'] = [
            'label' => esc_html__('Number of Shop Columns on Tablet', 'Divi'),
            'type' => 'select',
            'options' => [
                'off' => esc_html__('Default', 'Divi'),
            ...
            'default' => 'off',
            'description' => esc_html__('Select the number of product columns to show on Tablet.', 'Divi'),
            'tab_slug' => 'general',
            'toggle_slug' => 'main_content',
        ];
        $fields['pac_drh_column_numbers_phone'] = [
            'label' => esc_html__('Number of Shop Columns on Phone', 'Divi'),
            'type' => 'select',
            'options' => [
                'off' => esc_html__('Default', 'Divi'),
            ...
            'default' => 'off',
            'description' => esc_html__('Select the number of product columns to show on Phone.', 'Divi'),
            'tab_slug' => 'general',
            'toggle_slug' => 'main_content',
        ];
        $fields_unprocessed['columns_number'] = [];

        return array_merge($fields_unprocessed, $fields);
    }
}
add_filter('et_pb_all_fields_unprocessed_et_pb_shop', 'pac_drh_add_shop_col_numbering_options');