...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        mObserver.observe(htmlObj.get(0), {
            attributes: true,
            subtree: true,
            childList: true,
        });
        ...
        $('body').on('click', '.pac_preset_preview_button', (e) => {
            ...
            if (currentActiveValue) {
                ...
                jQuery('.pac_set_preview_size_input_filed').val(currentActiveValue);
            }
            var customValue = jQuery('.pac_set_preview_size_input_filed').val();
            if (customValue) {
                width = customValue;
            }
            ...
            jQuery('.pac_set_preview_size_input_filed').mouseleave(function () {
                setTimeout(function () {
                    var newPreviewSWize = jQuery('.pac_set_preview_size_input_filed').val();
                    if ((newPreviewSWize > 0) && (newPreviewSWize < 768)) {
                        if (isTabletPreview() || isDesktopPreview()) {
                            jQuery('.et-fb-icon--phone').click();
                            setTimeout(function () {
                                jQuery('.pac_set_preview_size_input_filed').val(newPreviewSWize).click();
                            }, 1200);
                        }
                    }
