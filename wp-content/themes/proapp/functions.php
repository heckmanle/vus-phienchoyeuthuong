...
function site_scripts() {
    ...
    wp_enqueue_style( 'theme_style_toastr', get_theme_file_uri( 'css/toastr.css' ), [], $version );
    wp_enqueue_style( 'cropper-css', get_theme_file_uri( 'assets/libs/cropper/css/cropper.css' ), [], $version );
    wp_enqueue_style( 'range-css', get_theme_file_uri( 'assets/libs/range/css/rangeslider.css' ), [], $version );
    // Theme stylesheet.
    wp_enqueue_style( 'site-style', get_stylesheet_uri() );
    // Load the stylesheet.
    wp_enqueue_style( 'main_admin', get_theme_file_uri( 'main.css' ), array( 'site-style' ), $version );
    wp_enqueue_style( 'theme_style', get_theme_file_uri( 'style.css' ), array( 'site-style' ), $version );

    wp_enqueue_style( 'jquery.ui', get_theme_file_uri( '/assets/libs/jquery/css/jquery-ui.min.css' ), array( 'site-style' ), $version );
    wp_enqueue_style( 'simplebar', get_theme_file_uri( '/assets/libs/simplebar/css/simplebar.css' ), array(  ), $version );

    wp_enqueue_style( 'theme_style_authen', get_theme_file_uri( 'css/authentication.css' ), array( 'site-style' ), $version );
    // Load the html5 shiv.
    ...
    wp_enqueue_script('helpers', THEME_URL . '/js/helpers.js', ['jquery'], $version);
    wp_enqueue_script('convert-to-money', THEME_URL . '/js/convert-money-to-text.js', ['jquery'], $version);
    wp_enqueue_script('popper-js', THEME_URL . '/assets/libs/bootstrap/js/popper.js', [], $version);
    wp_enqueue_script('jquery.validate', THEME_URL . '/assets/libs/jquery/js/jquery.validate.js', ['jquery', 'jquery-form'], $version);
    wp_enqueue_script('simplebar', THEME_URL . '/assets/libs/simplebar/js/simplebar.js', ['jquery', 'jquery-form'], $version);
    wp_enqueue_script('jquery.easing', THEME_URL . '/assets/libs/jquery/js/jquery.easing.1.3.js', ['jquery'], $version);
    wp_register_script('moment-front', THEME_URL . '/assets/libs/bootstrap/js/moment-with-locales.js', '', '');
    wp_register_script('recaptcha', 'https://www.google.com/recaptcha/api.js', '', '');
    wp_register_script('firebase', 'https://www.gstatic.com/firebasejs/8.1.2/firebase.js', '', '', ['recaptcha']);
    $firebaseConfig = [
        "apiKey" => "AIzaSyCcqEQhJTKxbGBaTQuwKBwKALFRPRZxINY",
        "authDomain" => "vus-webapp.firebaseapp.com",
        "databaseURL" => "https://vus-webapp-default-rtdb.asia-southeast1.firebasedatabase.app",
        "projectId" => "vus-webapp",
        "storageBucket" => "vus-webapp.appspot.com",
        "messagingSenderId" => "185727549101",
        "appId" => "1:185727549101:web:aa55741cedb43826829024",
        "measurementId" => "G-8B8LQ8TQ78"
    ];

    wp_localize_script('firebase', 'FIREBASE_CONFIG', $firebaseConfig);
    wp_enqueue_script('jquery.additional.methods', THEME_URL . '/assets/libs/jquery/js/additional-methods.min.js', ['jquery', 'jquery.validate', 'jquery.form'], $version);
    wp_enqueue_script('jquery.blockui', THEME_URL . '/assets/libs/jquery/jquery.blockUI.js', ['jquery', 'jquery.validate', 'jquery-form'], $version);
    wp_register_script('cropper-js', THEME_URL . '/assets/libs/cropper/js/cropper.js', ['jquery'], $version);
    wp_register_script('range-js', THEME_URL . '/assets/libs/range/js/rangeslider.js', ['jquery'], $version);
    wp_enqueue_script('select2-js', THEME_URL . '/assets/libs/select2/js/select2.full.js', ['jquery'], $version);
    wp_enqueue_script('bootstrap-js', THEME_URL . '/assets/libs/bootstrap/js/bootstrap.js', ['jquery', 'popper-js'], $version, false);
    wp_enqueue_script('bootstrap-util-js', THEME_URL . '/assets/libs/bootstrap/js/util.js', ['jquery', 'popper-js'], $version);
    wp_enqueue_script('bootstrap-modal-js', THEME_URL . '/assets/libs/bootstrap/js/modal.js', ['jquery', 'popper-js', 'bootstrap-util-js'], $version);
    wp_enqueue_script('bootstrap-tab-js', THEME_URL . '/assets/libs/bootstrap/js/tab.js', ['jquery', 'popper-js', 'bootstrap-util-js'], $version);
    wp_enqueue_script('toastr-js', THEME_URL . '/assets/libs/jquery/toastr.js', ['jquery'], $version);
    wp_enqueue_script('jquery-repeater', THEME_URL . '/assets/libs/jquery/jquery.repeater.js', ['jquery'], $version);
    wp_register_script('apexcharts', THEME_URL . '/assets/libs/apexchart/js/apexcharts.js', ['jquery'], $version);
    wp_enqueue_script('main-1-js', THEME_URL . '/assets/scripts/main.js', ['jquery'], $version);
    //wp_enqueue_script('main-2-js', THEME_URL . '/js/main.js', ['jquery'], $version);


    #wp_enqueue_script( 'jquery-js', get_theme_file_uri( '/vendor/jquery/jquery.min.js' ), array(), $version );
    wp_enqueue_script( 'bootstrap', get_theme_file_uri( '/vendor/bootstrap/js/bootstrap.bundle.min.js' ), array(), $version );
...
function _site_default_wp_die_handler($message){
...
define('RECAPTCHA_KEY', '6LdxNwIaAAAAAB-Enp0Z81N0_PtvawWNPqZMs0D9');
define('RECAPTCHA_SECRET', '6LdxNwIaAAAAACwLECuIZfhXDpv-e_SG5T8VXYeK');

require_once THEME_DIR . '/api/class.api.php';
require_once THEME_DIR . '/includes/ajax.php';
require_once THEME_DIR . '/inc/upload-file.php';
foreach (glob(THEME_DIR . '/includes/core/*.php') as $file_name){
    if( file_exists($file_name) ){
        require_once  $file_name;
    }
}
define('DECIMAL_SEP', '.');
define('THOUNSAND_SEP', ',');
define('NUM_DECIMAL', 0);
require_once THEME_DIR . "/includes/system.php";
require_once THEME_DIR . "/inc/helpers.php";
require_once THEME_DIR . "/inc/SiteExelFactory.php";
require_once THEME_DIR . "/inc/template-twig.php";
 ...
 */
/*global $system_api;
$add_product = $system_api->query(
    "POST",
    'addProductCategory',
    [
        'params' => [
            'cate_title' => 'Đèn led dài tròn 1',
            'cate_description' => 'abc',
            'cate_parent' => '5fc4f19fd4822b1b82ae83fa'],
        'fields' => ['cate_title', 'cate_description']],
    true
);*/
add_action( 'rest_api_init', 'qsm_register_rest_routes' );
function qsm_register_rest_routes() {
    register_rest_route( 'sme/v1', '/image/', array(
        'methods'  => WP_REST_Server::CREATABLE,
        'callback' => 'ns_rest_create_image',
    ) );
}
function ns_rest_create_image(WP_REST_Request $request){
    $file = $request->get_file_params();
    $file_image = $file['file_image'];

    $token = $request['token'];

    if(empty($file_image)){
        return new \WP_Error(403, "Không tìm thấy file");
    }
    $file_upload = new \SME\Inc\UploadFile($file_image, ['jpg|jpeg|jpe' => 'image/jpeg', 'png' => 'image/png',]);
    $fileImport = $file_upload->save_file();
    if(is_wp_error($fileImport)){
        return $fileImport;
    }
    $upload_dir = wp_get_upload_dir();
    $fileImport = str_replace($upload_dir['url'], $upload_dir['url'], $fileImport);
    wp_send_json(['url' => $fileImport]);
}
...
//define('THOUNSAND_SEP', ',');
//define('DECIMAL_SEP', '.');
require_once get_template_directory() . '/api/class.api.php';
foreach (glob(get_template_directory() . '/api/core/*.php') as $file_name){
    if( file_exists($file_name) ){
        require_once  $file_name;
    }
}
...
function et_setup_theme() {
    ...
    //require_once( $template_directory . '/includes/functions/helpers.php' );
...
function et_theme_epanel_reminder(){
    ...
    if ( false === et_get_option( $shortname . '_logo' ) && false === et_get_option( $documentation_option_name ) ) {
        $message = sprintf(
            et_get_safe_localization( __( 'Welcome to our theme! Before diving in to your new theme, please visit the  page for access to dozens of in-depth tutorials.', $themename ) ),
...
require_once get_template_directory() . '/core/module-component.php';
require_once get_template_directory() . '/includes/modules-component.php';
require_once get_template_directory() . '/includes/component.php';
