...
class AJAX {
    ...
    {
        add_action('wp_ajax_handle_ajax', [$this, 'handle_ajax_request']);
        add_action('wp_ajax_nopriv_handle_ajax', [$this, 'handle_ajax_request']);
    }
    ...
    function process_action($callback, $data){
        try{
            ...
            if( $result === false ){
                ...
                $data = apply_filters('app/ajax/insert_data_errors', []);
                $this->add_response_data($message, false, $data, !is_numeric($code) ? 401 : $code)
                    ->send_error($code, $message);
            }else{
                $this->add_response_data(__('Success'), true, $result);
            }
        }catch (\Exception $e){
    ...
    public function handle_ajax_action($tag, $callback){
        if ( ! did_action( 'app/ajax/register_actions' ) ) {
            _doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'app/ajax/register_actions' ) ), '1.0.0' );
        }
        $this->ajax_actions[$tag] = compact('tag', 'callback');
    }

    public function handle_nopriv_ajax_action($tag, $callback){
        if ( ! did_action( 'ajax/register_nopriv_actions' ) ) {
            _doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'ajax/register_nopriv_actions' ) ), '1.0.0' );
        }
