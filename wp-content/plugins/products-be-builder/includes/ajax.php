...
class AJAX {
    ...
    {
        add_action('wp_ajax_rep_handle_ajax', [$this, 'handle_ajax_request']);
    }
    ...
    function process_action($callback, $data){
        try{
            ...
            if( $result === false ){
                ...
                $data = apply_filters('rep/ajax/insert_data_errors', []);
    ...
    function handle_ajax_request(){
        ...
        if( is_user_logged_in() ){
	        do_action('rep/ajax/register_actions', $this);
    ...
    public function handle_ajax_action($tag, $callback){
        if ( ! did_action( 'rep/ajax/register_actions' ) ) {
            _doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'rep/ajax/register_actions' ) ), '1.0.0' );
        }
