jQuery(function( $ ) {
	...
	$.POSTSBB = {
		init() {
			this.$el = $('.layout-postsbb-create');
			this.$templatePostDetail = $('script#tpl-postsbb-detail');
			this.$templateGalleryItem = $('script#tpl-gallery-item');
			this.$templatePostItem = $('script#tpl-postsbb-item');
			this.backbone();
		},
		backbone() {
			const self = this;
			let _postsbb = Backbone.View.extend({
				el: self.$el,
				template: {
					post_detail: _.template(self.$templatePostDetail.html()),
					gallery_item: _.template(self.$templateGalleryItem.html()),
					post_item: _.template(self.$templatePostItem.html()),
				},
				gallery: {
					idx: 0,
					files: [],
				...
				events: {
					...
					'click .postsbb-product-item': 'eventShowPostDetail',
					'click .js-edit-pro-title': 'eventTogglePostTitle',
					'keyup .input-pro-title': 'eventKeyPressProTitle',
					'change .postsbb-js-pro-gallery-add input[type="file"]': 'eventGalleryAdd',
					'click .js-pro-gallery-item-remove': 'eventGalleryRemove',
					'click .postsbb-section-pagination ul.pagination a.page-numbers': 'eventPage',
					'click .handle-actions .handlediv': 'eventToggleDiv',
					'click #postsbb-btn-pro-action-delete': 'eventDelete',
				},
				initialize() {
					this.initEvents();
					this.initForm();
				...
				initForm () {
					const _self = this;
					$('form.postsbb-form-product').each(function(idx, ele){
				...
				formSubmit ($form, validator, ev) {
					...
					let options = {
						...
						beforeSend: function () {
							$('body').block(POSTSBB_IMAGE_LOADING);
						},
						success: function(response, status, xhr){
							let {edit_post_url, id} = response.data;
							$('.postsbb-builder-layout').removeClass('d-none');
							$('.postsbb-builder-layout .postsbb-drag-drop-layout').attr('href', edit_post_url);
						...
						complete: function(xhr, status){
							$('body').unblock(POSTSBB_IMAGE_LOADING);
							return;
						}
					};
				...
				async loadList() {
					let res = {};
					let options = {
						url: POSTSBB_AJAX_URL,
						type: 'get',
						dataType: 'json',
						async: true,
						data: {action: 'postsbb_handle_ajax', func: 'postsbb_get_my_posts', page: POSTSBB_GLOBAL.page},
						beforeSend: function (xhr, settings) {
							$('#postsbb-product-tabs-content').block(POSTSBB_IMAGE_LOADING);
							return;
						},
						error: function (xhr, status, errThrow) {
						...
						complete: function (xhr, status) {
							$('#postsbb-product-tabs-content').unblock(POSTSBB_IMAGE_LOADING);
							return;
						}
					}
					await $.ajax(options);
				...
				async getPostDetail(id) {
					...
					let options = {
						...
						dataType: 'json',
						async: true,
						data: {action: 'postsbb_handle_ajax', func: 'postsbb_get_post_by_id', id: id},
						beforeSend: function (xhr, settings) {
							//$('.postsbb-form-product').block(POSTSBB_IMAGE_LOADING);
							return;
						},
						error: function (xhr, status, errThrow) {
						...
						complete: function (xhr, status) {
							//$('.postsbb-form-product').unblock(POSTSBB_IMAGE_LOADING);
							return;
						}
					}
					await $.ajax(options);
				...
				addEvents($ele) {
					const self = this;
					$('select.postsbb-select-pro-cates', $ele).select2({
						data: POSTSBB_GLOBAL.post_categories.map((item, idx) => {item.text = item.post_cate_title; item.id = item.id; return item;})
					});
					let {selectedDefault} = $('select.postsbb-select-pro-cates', $ele).data();
					if( selectedDefault ){
						let selected = [];
						selectedDefault.forEach(function (item, idx){
							selected.push(item.id);
						});
						$('select.postsbb-select-pro-cates', $ele).val(selected).trigger('change');
					}
					let $select_pro_status = $('select.postsbb-status', $ele), {selectedDefault: proStatus} = $select_pro_status.data();
				...
				eventShowPostDetail(ev) {
					...
					p.then(function(data) {
						...
						let html = _self.templateParse('post_detail', data);
						$('.postsbb-form-product').unblock(POSTSBB_IMAGE_LOADING);
						$('.postsbb-pro-detail').html(html);
						_self.addEvents($('.postsbb-pro-detail'));
					}, function(error){
						console.log(error);
					});
					p.catch((e) => {
				...
				eventGalleryAdd(ev) {
					const _self = this;
					let $this = $(ev.currentTarget), files = ev.currentTarget.files, $parent = $this.closest('.postsbb-pgi-add');
					let validImageTypes = ['image/gif', 'image/jpeg', 'image/png'];
					let _length = $('.postsbb-pgi-content .postsbb-pgi:not(.postsbb-pgi-add)').length;
					if( _length >= POSTSBB_GLOBAL.limit_file ){
						$parent.addClass('d-none');
						_self.trigger('pushNotification', 'The number of images is not more than ' + POSTSBB_GLOBAL.limit_file, 'warning');
						return;
					}
					let html = '', i = 0;
					$.each(files, function (index, file) {
						if( i === POSTSBB_GLOBAL.limit_file ){
							$parent.addClass('d-none');
							return;
						}
					...
					if( $('.postsbb-pgi-content .postsbb-pgi:not(.postsbb-pgi-add)').length >= POSTSBB_GLOBAL.limit_file ){
						$parent.addClass('d-none');
					}
				},
				eventGalleryRemove(ev) {
					const _self = this;
					let $this = $(ev.currentTarget), $parents = $this.closest('.postsbb-pgi-content'), $parent = $this.closest('.postsbb-pgi'), $add = $parents.find('.postsbb-pgi-add'), {idx} = $parent.data();
					...
					this.createModal({
						...
						funcYes: () => {
							let options = {
								...
								dataType: 'json',
								async: true,
								data: {action: 'postsbb_handle_ajax', func: 'postsbb_delete_post_by_id', id: id},
								beforeSend: function (xhr, settings) {
									$('.postsbb-form-product').block(POSTSBB_IMAGE_LOADING);
									return;
								},
								...
								success: function (response, status, xhr) {
									...
									_self.render();
									$('.postsbb-pro-detail').html('<p class="alert_aligncenter">Please add new page or choose page for update ..</p>');
				...
				render() {
					...
					p.then(function(data) {
						...
						$.each(data.result, function (idx, item){
							html += _self.templateParse('post_item', Object.assign({}, _self.defaults, item));
						});
						$('#postsbb-myposts ul').html(html);
						$('#postsbb-myposts .postsbb-section-pagination').html(data.pagination);
					}, function(error){
						console.log(error);
					});
					p.catch((e) => {
			...
			this.postsbb = new _postsbb();
			this.postsbb.on('pushNotification', function(message, type) {
